#!/usr/bin/env bash
# Runs the same deploy.sh step as the GitHub Actions backend-deploy job.
# Note: the CI job also polls the public VITE_API_BASE/health endpoint after
# deploy.sh; set VITE_API_BASE in .env.deploy to enable that check here too.
