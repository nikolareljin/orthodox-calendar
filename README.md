...
- Script helpers come from git submodule `script-helpers` (`git@github.com:nikolareljin/script-helpers.git`). Run `./update` (or `git submodule update --init --recursive`) before using scripts; a local reference is also copied into `script-helpers`.
